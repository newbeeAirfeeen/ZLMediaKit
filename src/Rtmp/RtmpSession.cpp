...
RtmpSession::~RtmpSession() {
    ...
    // 另外我们再销毁的时候
    if (_push_src) {
        AuthCenter::instance().unreigst_auth(_media_info);
    }
}
...
void RtmpSession::onError(const SockException &err) {
    ...
    WarnP(this) << (is_player ? "RTMP播放器(" : "RTMP推流器(") << _media_info.shortUrl() << ")断开:" << err.what()
                << ",耗时(s):" << duration;
    ...
    if (_total_bytes >= iFlowThreshold * 1024) {
        NoticeCenter::Instance().emitEvent(
            Broadcast::kBroadcastFlowReport, _media_info, _total_bytes, duration, is_player,
...
void RtmpSession::onCmd_publish(AMFDecoder &dec) {
    ...
    if (!this->_hook_url.empty() && !this->_stream_proxy_session_id.empty()) {
        InfoL << "hook_url:" << this->_hook_url << ", _stream_proxy_session_id:" << this->_stream_proxy_session_id << ", app: " << _media_info._app << ", streamid: " << _media_info._streamid;
        // 我们在这里注册动态鉴权事件
        AuthCenter::instance().regist_auth(_media_info, this->_hook_url, this->_stream_proxy_session_id);
    }
    ...
    auto on_res = [this, token, now_stream_index](const string &err, const ProtocolOption &option) {
        ...
        if (push_failed) {
            sendStatus({ "level", "error", "code", "NetStream.Publish.BadName", "description", "Already publishing.",
                         "clientid", "0" });
        ...
        sendStatus({ "level", "status", "code", "NetStream.Publish.Start", "description", "Started publishing stream.",
                     "clientid", "0" });
    ...
    Broadcast::PublishAuthInvoker invoker
        = [weak_self, on_res, token](const string &err, const ProtocolOption &option) {
              auto strong_self = weak_self.lock();
              if (!strong_self) {
                  return;
              }
    ...
    auto flag = NoticeCenter::Instance().emitEvent(
        Broadcast::kBroadcastMediaPublish, MediaOriginType::rtmp_push, _media_info, invoker,
        static_cast<SockInfo &>(*this));
    if (!flag) {
        // 该事件无人监听，默认鉴权成功
...
void RtmpSession::sendPlayResponse(const string &err, const RtmpMediaSource::Ptr &src) {
    ...
    sendStatus(
        { "level", (ok ? "status" : "error"), "code",
          (ok ? "NetStream.Play.Reset" : (auth_success ? "NetStream.Play.StreamNotFound" : "NetStream.Play.BadAuth")),
          "description", (ok ? "Resetting and playing." : (auth_success ? "No such stream." : err.data())), "details",
          _media_info._streamid, "clientid", "0" });
    ...
    sendStatus({ "level", "status", "code", "NetStream.Play.Start", "description", "Started playing.", "details",
                 _media_info._streamid, "clientid", "0" });
    ...
    sendStatus({ "level", "status", "code", "NetStream.Play.PublishNotify", "description", "Now published.", "details",
                 _media_info._streamid, "clientid", "0" });
...
void RtmpSession::onCmd_pause(AMFDecoder &dec) {
    ...
    sendStatus({ "level", "status", "code", (paused ? "NetStream.Pause.Notify" : "NetStream.Unpause.Notify"),
...
void RtmpSession::onRtmpChunk(RtmpPacket::Ptr packet) {
    ...
    switch (chunk_data.type_id) {
        ...
        default:
            WarnP(this) << "unhandled message:" << (int)chunk_data.type_id
                        << hexdump(chunk_data.buffer.data(), chunk_data.buffer.size());
            break;
    }
}
